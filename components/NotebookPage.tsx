...
import { ArrowLeftIcon, NotebookIcon, SendIcon, SparklesIcon, ThumbsUpIcon, ThumbsDownIcon, PaperClipIcon } from './icons/GeneralIcons';
...
const NotebookPage: React.FC<NotebookPageProps> = ({ onBack, sources, setSources }) => {
  ...
  return (
    <div className="p-4 md:p-6 bg-glass-light dark:bg-glass-dark border border-border-light dark:border-border-dark rounded-2xl shadow-xl backdrop-blur-3xl animate-fade-in transition-all duration-300 ease-out hover:shadow-2xl hover:-translate-y-1 flex flex-col h-[calc(100vh-7rem)]">
      ...
      {/* Main Content: Chat Panel taking full width */}
      <div className="flex-grow flex flex-col bg-white/10 dark:bg-black/10 rounded-lg overflow-hidden">
        <div className="flex-grow overflow-y-auto p-4 space-y-4">
            ...
            {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center text-gray-600 dark:text-gray-400 p-4">
                    ...
                    <p className="text-lg">{t('selectSourcesPrompt')}</p>
                </div>
            )}
            ...
            <div className="px-4 pb-3 flex flex-wrap justify-start items-center gap-2 animate-fade-in">
              {suggestions.map((q, i) => (
                <button
                  ...
                  className="px-4 py-2 text-base bg-white/50 dark:bg-black/20 text-gray-800 dark:text-gray-200 rounded-full border border-border-light dark:border-border-dark hover:bg-white/80 dark:hover:bg-black/30 transition-colors"
                >
                  {q}
                </button>
              ))}
